...
(async () => {
    ...
    gas = await mycontract.estimateGas.supply(process.env.AAVEpool, process.env.USDC, ethers.BigNumber.from('700000'), mycontract.address);
    receipt = await mycontract.supply(process.env.AAVEpool, process.env.USDC, ethers.BigNumber.from('700000'), mycontract.address, {
        gasLimit: ethers.BigNumber.from(gas).mul(ethers.BigNumber.from('2'))
    });
    await receipt.wait();
    qwe = await mycontract.getUserAccountData(process.env.AAVEpool, mycontract.address);
